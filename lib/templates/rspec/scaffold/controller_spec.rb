...
<% unless options[:singleton] -%>
  describe "GET index" do
    it "assigns all <%= table_name.pluralize %> as @<%= table_name.pluralize %>" do
      <%= stub "#{class_name}.accessible_by" %> { [<%= mock_file_name %>].paginate }
      get :index
      assigns(:<%= table_name %>).should eq([<%= mock_file_name %>].paginate)
    end
  end
  ...
  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested <%= file_name %>" do
        <%= stub orm_class.find(class_name) %> { <%= mock_file_name %> }
        mock_<%= should_receive orm_instance.update_attributes %>
